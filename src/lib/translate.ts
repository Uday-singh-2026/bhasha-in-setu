...
const normalise = (s: string) => {
  ...
  // Normalize spoken Hinglish self-introductions transcribed by speech recognizer:
...
function lookup(word: string): Entry | undefined {
  ...
  // Common Hindi verb, pronoun and noun inflections
  const stems = [
    // Future verbal suffixes
    word.replace(/(ूंगा|ूंगी|ेगा|एगी|ेंगे|ेंगी)$/u, ""),
    // Habitual / participle / infinitive suffixes
    word.replace(/(ता|ती|ते|ना)$/u, ""),
    // Past participle suffixes
    word.replace(/(ाया|ाये|ाई)$/u, ""),
    // Plural / oblique suffixes
    word.replace(/(ों|ें|ाँ|ां)$/u, ""),
    // Generic matra endings
    word.replace(/(ो|े|ा|ी)$/u, ""),
    // Postposition attached to words
...
/**
 * Offline Hindi -> tribal-language engine.
 * 1. phrase memory (whole utterance)
 * 2. longest-match n-gram phrase chunks
 * 3. word-level lexicon with light stemming & automatic name/loanword transliteration
 */
export function translate(input: string, lang: LangCode): TranslationResult {
  ...
  while (i < words.length) {
    ...
    if (entry) {
      ...
      // Intelligently transliterate unknown words/names into target script
